package cal_on.wastmanagement.NewDatabase;
...
@SuppressLint("NewApi")
public class DBHelperreports extends SQLiteOpenHelper {

	DBHelperreports mydbhelper;
	SQLiteDatabase mydatabase;
	Context mycontext;
	public DBHelperreports(Context context, String name, CursorFactory factory,
                    int version) {
		
		super(context, Database.CAl_DATABASE_NAME, null, Database.CAL_DATABASE_VERSION);
		mycontext=context;
	...
	public void onCreate(SQLiteDatabase db) {
		...
		String bi_cust_table_sql = "create table " + Database.BI_Cust_TABLE_NAME + " ( " +
				Database.BI_Cust_BILL_NO	+ " integer  primary key autoincrement );";

		db.execSQL(bi_cust_table_sql );

		String bi_item_table_sql = "create table " + Database.BI_Item_TABLE_NAME + " ( " +
				Database.BI_Item_BILL_NO	+ " integer  primary key autoincrement );";

		db.execSQL(bi_item_table_sql );
	}
